...
if __name__ == "__main__":
    ...
    for transformer_name, (transformer, last_child) in TRANSFORMERS.items():
        ...
        for max_len in max_lengths:
            ...
            accuracies_mean.append(np.mean(accuracy_list))
            accuracies_std.append(np.std(accuracy_list))

    accuracies_mean_nparray = np.array(accuracies_mean)
    accuracies_std_nparray = np.array(accuracies_std)
    # Plot results:
    plt.plot(max_lengths[:-1] + [65], accuracies_mean)
    plt.fill_between(max_lengths[:-1] + [65],accuracies_mean_nparray-accuracies_std_nparray,accuracies_mean_nparray+accuracies_std_nparray)
    plt.xlim(25, 65)
